...
计算价值函数看起来难以处理——它们被定义为对无限多未来轨迹的期望。在实践中，RL使用三种方法：**动态规划**（基于模型，迭代贝尔曼更新）、**蒙特卡洛**（采样完整回合，平均回报）和**时序差分**（使用当前估计进行自举）。对于**Q函数**，同样的方法适用，但我们学习 $Q(s,a)$ 对，通过 $\arg\max_a Q(s,a)$ 实现无模型控制。**优势函数** $A(s,a) = Q(s,a) - V(s)$ 衡量一个动作比平均好多少——对策略梯度至关重要。对于大状态空间，**函数逼近**在状态间进行泛化。

**需要记忆的**：V和Q的DP/MC/TD方法，优势定义及其如何减少方差，TD误差作为优势估计，GAE用于偏差-方差权衡。
...
**V(s) 告诉你一个状态有多好，但不告诉你该采取哪个动作。**

要最优地行动，你需要比较动作：

$$\pi^*(s) = \arg\max_a Q^*(s, a)$$

有了 $Q(s,a)$，你可以直接选择最佳动作。只有 $V(s)$ 时，你需要模型：

$$\pi^*(s) = \arg\max_a \left[ R(s,a) + \gamma \sum_{s'} P(s'|s,a) V^*(s') \right]$$

**关键洞见**：Q学习和DQN直接学习 $Q$，实现无模型控制！

### Q函数定义

$$Q^\pi(s, a) = \mathbb{E}_\pi\left[\sum_{t=0}^{\infty} \gamma^t r_t \mid s_0 = s, a_0 = a\right]$$

**与V的区别**：第一个动作 $a$ 是指定的，然后遵循 $\pi$。
...
**复杂度**：$O(|S|^2 |A|^2)$——比计算V更昂贵！

#### Q的蒙特卡洛

```
初始化 Q(s,a) = 0, counts(s,a) = 0
...
**关键特性**：使用 $\max_{a'} Q(s', a')$——无论探索策略如何，都学习最优Q！
...
经过多个回合后：
  Q(S,R) → 5    （立即奖励，然后终止）
  Q(S,L) → 1 + 0.9×5 = 5.5  （现在1 + 折扣后的5）

最优策略：先采取 L（Q(S,L) > Q(S,R)）！
```
...
|-----------|-------|-------|
| **有模型** | ✓（可推导策略） | ✓（冗余但可行） |
| **无模型，需要策略** | ✗（无法选择动作） | ✓（argmax给出策略） |
| **连续动作** | ✓（用策略梯度） | ✗（无法枚举max） |
| **内存有限** | ✓（$|S|$ 个值） | ✗（$|S| \times |A|$ 个值） |

**在实践中**：
- DQN, Q学习 → 学习 Q
- Actor-Critic, PPO → 学习 V（critic）+ 策略（actor）
...
**含义**：动作 $a$ 比状态 $s$ 中的平均动作好多少？

- $A(s,a) > 0$：动作 $a$ **优于**平均
- $A(s,a) < 0$：动作 $a$ **劣于**平均
- $A(s,a) = 0$：动作 $a$ 恰好是平均水平

### 为什么优势重要

**策略梯度中使用原始Q值的问题**：

考虑一个所有动作都给出高回报的状态（例如接近目标）：
- $Q(s, \text{left}) = 95$
- $Q(s, \text{right}) = 100$

直接使用Q，策略梯度会增加**两个**动作的概率（两者Q都是正的）。但right明显更好！

**优势修复了这个问题**：
- $V(s) = 97.5$（平均值）
- $A(s, \text{left}) = 95 - 97.5 = -2.5$ → 降低概率
- $A(s, \text{right}) = 100 - 97.5 = +2.5$ → 增加概率
...
TD误差是优势的**无偏估计**！只需要V，不需要Q。

**这就是Actor-Critic工作的原因**：
```
Actor：使用优势（TD误差）更新策略
Critic：只学习 V(s)——比学习 Q(s,a) 简单得多
```
...
| $\lambda$ | 行为 |
|-----------|----------|
| 0 | TD(0)：$\hat{A} = \delta_t$（低方差，高偏差） |
| 1 | MC：$\hat{A} = G_t - V(s_t)$（高方差，低偏差） |
| 0.95-0.99 | 两全其美（实践中使用） |

### 优势在算法中的应用

| 算法 | 如何使用优势 |
|-----------|----------------------|
| **A2C/A3C** | 策略梯度用TD误差 $\delta_t$ 加权 |
| **PPO** | 裁剪目标使用GAE优势 |
| **TRPO** | 带KL约束的更新使用优势 |
| **SAC** | 带熵奖励的软优势 |
...
<details markdown="1">
<summary><strong>问题7（概念）：</strong> 为什么我们需要Q函数而不仅仅是V函数？</summary>

**答案**：Q函数实现无模型控制——在不知道环境动态的情况下选择动作。

**解释**：
- 仅有 $V(s)$，要选择动作你需要：$\arg\max_a [R(s,a) + \gamma \sum_{s'} P(s'|s,a) V(s')]$
- 这需要模型（$P$ 和 $R$）！
- 有 $Q(s,a)$，动作选择很简单：$\arg\max_a Q(s,a)$
- 不需要模型！

**关键方程**：$\pi^*(s) = \arg\max_a Q^*(s,a)$

**常见陷阱**：认为V总是足够的。对于无模型控制，Q是必不可少的。这就是为什么Q学习和DQN如此重要。
</details>

<details markdown="1">
<summary><strong>问题8（概念）：</strong> 为什么在策略梯度中使用优势而不是Q能减少方差？</summary>

**答案**：优势减去了状态的"平均好坏程度"，所以只有动作的相对质量重要。

**解释**：
考虑一个接近目标的状态，所有动作都有高Q值：
- $Q(s, a_1) = 95$，$Q(s, a_2) = 100$

直接使用Q：两个动作都得到正更新（两者Q > 0），即使 $a_1$ 更差！

使用优势，$V(s) = 97.5$：
- $A(s, a_1) = 95 - 97.5 = -2.5$ → 概率降低
- $A(s, a_2) = 100 - 97.5 = +2.5$ → 概率增加

**关键性质**：$\mathbb{E}[A(s,a)] = 0$——优势不会使梯度产生偏差，只减少方差。

**关键方程**：$A(s,a) = Q(s,a) - V(s)$

**常见陷阱**：认为需要Q和V两个网络。在actor-critic中，TD误差 $\delta = r + \gamma V(s') - V(s)$ 是优势的无偏估计——只需要V！
</details>
...
**面试需要记忆的**：V和Q的DP/MC/TD方法，优势公式及其如何减少方差，TD误差作为优势估计，GAE公式，为什么Q能实现无模型控制，偏差-方差权衡。
